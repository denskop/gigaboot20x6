...
EFI_STATUS efi_main(EFI_HANDLE img, EFI_SYSTEM_TABLE* sys)
{
    ...
    ret = gBS->LocateHandle(ByProtocol, &ManagedNetworkProtocol, NULL, &size, handles);
    if (ret != EFI_SUCCESS) Print(L"Error reading MNP handles: %lu\n", ~EFI_ERROR_MASK & ret);
    else Print(L"Found %u handle(s)\n", size / sizeof(EFI_HANDLE));
